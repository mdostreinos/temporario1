...
try {
    ...
    $company_id = filter_input(INPUT_POST, 'company_id', FILTER_VALIDATE_INT); // Novo campo
    // Campos de instituição removidos:
    // $institution_name = trim($_POST['institution_name'] ?? '');
    // $institution_cnpj = trim($_POST['institution_cnpj'] ?? '');
    // ... e outros campos de endereço e contato da instituição
    $signature_data_base64 = $_POST['signature_data'] ?? '';
    $item_ids_str_for_donation = trim($_POST['item_ids_for_donation'] ?? '');
    // O nome do responsável pela instituição que assina ainda pode ser necessário,
    // mas ele agora é parte da entidade "Empresa" ou pode ser um campo separado no termo se for dinâmico.
    // Por ora, a assinatura é do "responsável da instituição", que pode ser buscado da tabela `companies`.
    // Se a pessoa que assina PODE SER DIFERENTE do responsável cadastrado na empresa,
    // um campo "Nome do Signatário" seria necessário no formulário do termo.
    // Para este escopo, vamos assumir que a assinatura é do responsável cadastrado na `companies`.
    ...
    if (empty($company_id) || $company_id === false) { // Validar company_id
        $errors[] = "Empresa/Instituição recebedora é obrigatória.";
    }
    // Validações para campos de instituição removidas
    // if (empty($institution_name)) $errors[] = "Nome da instituição é obrigatório.";
    // if (empty($institution_cnpj)) $errors[] = "CNPJ da instituição é obrigatório.";
    ...
    // Os campos institution_name e institution_responsible_name não são mais preenchidos diretamente aqui.
    // Eles permanecem na tabela para dados legados, mas serão NULL para novas doações com company_id.
    // O script 003_add_companies_table.sql já tornou essas colunas NULLABLE.
    $sql_insert_term = "INSERT INTO donation_terms (
        user_id, responsible_donation, donation_date, donation_time, company_id,
        signature_image_path, status
        -- institution_name, institution_responsible_name (deixados como NULL)
    ...
    // Note que $institution_name e $institution_responsible_name foram removidos do bind_param
    $stmt_insert_term->bind_param("isssis",
        $user_id_from_session,
        $responsible_donation_from_session,
        $donation_date,
        $donation_time,
        $company_id, // Novo campo
        $signature_db_path
    );

...
// Não requer admin estrito aqui, pois pode ser usado por usuários logados
// que estão criando termos de doação. Apenas garante que o usuário está logado.
// Se a criação de termos de doação for restrita a admins, então require_admin() seria apropriado.
// Por agora, vamos assumir que qualquer usuário logado que possa acessar a página de doação pode buscar empresas.
require_login();
...
if ($exact_id !== null) {
    ...
    if ($company_id) {
        $sql_exact = "SELECT id, name, cnpj, responsible_name, phone, email,
        ...
        if ($stmt_exact->execute()) {
            ...
            if ($company_data = $result_exact->fetch_assoc()) {
                ...
                $text_preview = htmlspecialchars($company_data['name']);
                if (!empty($company_data['cnpj'])) {
                    $text_preview .= " (CNPJ: " . htmlspecialchars($company_data['cnpj']) . ")";
                }
                $response['results'][] = [
                    'id' => $company_data['id'],
                    'text' => $text_preview, // Para consistência com Select2, se necessário
                    'name' => htmlspecialchars($company_data['name']),
                    'cnpj' => htmlspecialchars($company_data['cnpj']),
                    'full_data' => [ // Estrutura para os detalhes completos
                        'responsible_name' => htmlspecialchars($company_data['responsible_name'] ?? ''),
                        'phone' => htmlspecialchars($company_data['phone'] ?? ''),

...
if (!empty($filter_status) && in_array($filter_status, ['Pendente', 'Devolvido', 'Doado', 'Aguardando Aprovação', 'Perdido'])) {
...
// NOVO: Adiciona a condição de filtro para o código de barras
...
// NOVO: Bloco para buscar todos os IDs que correspondem ao filtro, sem paginação.
if (isset($_GET['get_all_ids']) && $_GET['get_all_ids'] === 'true') {
    // A consulta para IDs é mais simples, pois não precisa de todos os JOINs de dados, apenas os de filtro.
...
if ($stmt === false) {
    ...
    if (!empty($main_sql_params_types) && count($main_sql_params_values) > 2) { // 2 for LIMIT and OFFSET
        $bind_params = [];
        $bind_params[] = &$main_sql_params_types;
        foreach ($main_sql_params_values as $key => $value) {
            $bind_params[] = &$main_sql_params_values[$key];
        }

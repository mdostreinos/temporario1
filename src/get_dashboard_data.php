...
$sql_donations_institution = "SELECT dt.institution_cnpj, dt.institution_name, COUNT(dti.item_id) as total_items FROM donation_terms dt JOIN donation_term_items dti ON dt.term_id = dti.term_id JOIN items i ON dti.item_id = i.id {$don_timeline_conditions['sql']} GROUP BY dt.institution_cnpj, dt.institution_name ORDER BY total_items DESC LIMIT 10";
...
if ($stmt_don_inst && (!empty($don_timeline_conditions['params']) ? $stmt_don_inst->bind_param($don_timeline_conditions['types'], ...$don_timeline_conditions['params']) : true) && $stmt_don_inst->execute()) {
    ...
    while ($row = $result->fetch_assoc()) $dashboard_data['donations_by_institution'][] = $row;
    $stmt_don_inst->close();
